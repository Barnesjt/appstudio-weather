...
Item {
    ...
    function getDBRecent() {
      ...
      if(query.first()) {
        weatherDB.jsonString = query.values.strRes;
      ...
      for(var ok = query.first(); ok; ok = q.next()){
    ...
    function putDB(time, lat, lon, currTemp, days) {
      ...
      for(var i = 0; i<days.length;i++){
        var insertString = "INSERT INTO weather_days (time, max, min, strRes) VALUES ('"+days[i].time+"','"+days[i].max+"','"+days[i].min+"')";
        db.query(insertString);
      }
      db.close();
    }
    ...
    function urlFromCoords(lon, lat) {
      return "https://api.open-meteo.com/v1/forecast?latitude=" + lat + "&longitude=" + lon + "&daily=weathercode,temperature_2m_max,temperature_2m_min,&current_weather=true&temperature_unit=fahrenheit&windspeed_unit=mph&timeformat=unixtime&timezone=America%2FLos_Angeles";
    }

    //Make a purpose build object array from the api response for the daily forecasts
    function compileDays(var input){
      var days = [];
      ...
      onReadyStateChanged: {
        if(readyState == NetworkRequest.DONE){
          if(errorCode !== 0) {
            ...
            putDB(Date.now(), weatherDB.latitude, weatherDB.longitude, response.current_weather.temperature, compileDays(response));
            getDBRecent();
          }
        }
      }
